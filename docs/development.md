...
asks for the agent beacon URL, total Linux and Windows agent counts, and one
label per unique identity before the remaining server options. For unattended
count-based setup, use `--linux-agents <count>` and
`--windows-agents <count>`. The setup workflow builds a reusable `sablectl`
binary before it finishes.
...
sablectl rebuild --agent linux01  # just one agent label
```
...
```sh
pkill -f agent && scp builds/linux01/agent-linux user@target:/tmp/agent
...
| `--linux-agents` | `sablectl setup`, `sablectl install` | Desired total of unique Linux identities and binaries. |
| `--windows-agents` | `sablectl setup`, `sablectl install` | Desired total of unique Windows identities and binaries. |
