...
- `sablectl down` prefers the JWT-authenticated shutdown endpoint on the loopback-only operator API and waits for both the listener and managed process to exit. If certificate or credential state is unavailable, it may terminate only the PID previously recorded by `sablectl up`; it never reports success merely because pinned HTTPS verification failed.
