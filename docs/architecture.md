...
Persistent snapshots are coalesced and written outside the session lock. Large artifact bodies live in separate protected blobs so they do not force the complete JSON state to be recopied on every update. `--state-key-file` enables AES-256-GCM encryption for both the state file and artifact blobs.
